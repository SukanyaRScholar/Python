...
while continueOption != 'x': # Start the loop and allow user to enter different options
    ...
    if user_input[0] not in tbl.keys():  # if the user defined input not in the pre existing table numbers
        print ("Invalid table; Please re-enter")
    ...
    if user_input[1].startswith('P'):
        ...
        # order table command calls the takeorder method of class table
    ...
    elif user_input[1].startswith('C'):  # if the customer closes the order, the bill is generatyed
        tableObject.checkoutOrder(selectedTable, tableObject.partyCount)

    # this loops through until the user enters x. clicking anyother key keeps the program running
